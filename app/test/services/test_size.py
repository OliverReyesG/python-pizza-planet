...
def test_get_size_by_id_service(client, create_size, size_uri):
    ...
    fetched_size = response.json
    for param, value in current_size.items():
        pytest.assume(fetched_size[param] == value)


def test_list_sizes_service(client, create_sizes, size_uri):
    response = client.get(size_uri)
    pytest.assume(response.status.startswith('200'))
    fetched_sizes = {size['_id']: size for size in response.json}
    for size in create_sizes:
        pytest.assume(size['_id'] in fetched_sizes)
