...
def test_get_order_by_id_service(client, create_order, order_uri):
    ...
    for param, value in current_order.items():
        pytest.assume(fetched_order[param] == value)


def test_list_orders_service(client, create_orders, order_uri):
    response = client.get(order_uri)
    pytest.assume(response.status.startswith('200'))
    fetched_order = {order['_id']: order for order in response.json}
    for order in create_orders:
        pytest.assume(order.json['_id'] in fetched_order)
